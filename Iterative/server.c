 ...
 * Assignment: TFTP Server (Part 2 - Iterative)
 * Student Name: Brian Faure

Simple implementation of a Trivial FTP server. Prints all requests to terminal,
safisfies RRQ requests and ignores all others. Uses an iterative approach, can
only handle a single client at a time. The search_for_file function that is used to check
...
// Searches the working directory for the specified filename, if allow_global is
// 1, the filename is allowed to be anywhere on the filesystem
...
int main(int argc, char ** argv)
{
	...
	while(1)
	{
		...
		if (n<0){  printf("WARNING: Received invalid request.\n");  }
...
void send_error_packet(int err_num, char* err_msg, struct sockaddr_in* client_addr, socklen_t* addrlen)
{
	...
	// creating a struct to hold socket timeout options
	struct timeval tv;
